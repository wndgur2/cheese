...
async function getData(url, setBranches) {
  ...
  try{
    ...
    const promises = res.data.map(async (branch)=>{
      const newBranch = new Branch(branch.branchId, branch.name, branch.longitude, branch.latitude, branch.shootingCost, branch.printingCost, branch.paperAmount);
  ...
  return (
    <div className="container" style={{overflowY:"scroll", height:"calc(96vh - 64px)"}}>
      ...
      {branches.map((branch, i)=>{
        return (
          <div key={i}>
            <TextBtn href={`/home?branchId=${branch.id}`} color="#FFD56A"
