...
export default function Shares({shares, userData, setUserData, branches}) {
  ...
  const saveSelected = ()=>{
    const data = selected.map((i)=>Object.values(shares[i].sharedPhotoMap).map((v)=>"data:image/png;base64," + v.photoImage));
  ...
  const deleteSelected = ()=>{
    ...
    selected.map((i)=>{deleteShare(shares[i].shareId, session)});
  ...
  const renderImage = (i)=>{
    ...
    result.push(
      <td key={i}
        ...
        <div className={`${myCheeseStyles.imageWrapper} ${selected.includes(i)?myCheeseStyles.selected:""}`}>
          <img
  ...
  useEffect(()=>{
    ...
    const currentDate = new Date(shares[selected[0]].createdAt).toUTCString().split(' ').slice(0, 4).join(' ');
    for(let i=1; i<selected.length; i++){
      if(shares[selected[i]].branchId != currentBranchId){
        setIsAllSameBranch(false);
      }
      if(new Date(shares[selected[i]].createdAt).toUTCString().split(' ').slice(0, 4).join(' ') != currentDate){
              ...
              marginBottom:"3vh",
                ...
                <span style={{
            ...
            }}>
                  {
                    isAllSameBranch &&
                    branches.map((b)=>{
                      if(b.branchId == shares[selected[0]].branchId) return b.name;
                    }).join('')
                  }
                </span>
              {
                ...
                <span style={{
                  ...
                  fontWeight: 300,
                  color: "#444",
                }}>{new Date(shares[selected[0]].createdAt).toUTCString().split(' ').slice(0, 4).join(' ')}</span>
              }
            </div>
          }
          <div style={{
