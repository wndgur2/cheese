...
import deleteTimelapse from "@/api/deleteTimelapse";
import { useSession } from "next-auth/react";
import saveTimelapseOnDevice from "@/api/saveTimelapsesOnDevice";
...
export default function Timelapse({timelapses, setUserData, userData}) {
  ...
  const saveSelected = ()=>{
    const data = selected.map((i)=>("data:video/mpeg;base64," + timelapses[i].video));
  ...
  const deleteSelected = ()=>{
    ...
    selected.map((i)=>{deleteTimelapse(timelapses[i].timelapseId, session)});
      ...
      <td align="center" key={i}
        onClick={()=>{handleImageClick(i)}}>
          <div className={
            ...
            >
              <source src={"data:video/mpeg;base64," + timelapses[i].video + "#t=0.3"} type="video/webm" />
            </video>
          </div>
      </td>
    );
