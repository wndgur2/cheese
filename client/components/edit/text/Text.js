...
import { Noto_Sans_KR, Roboto, Nanum_Myeongjo, Nanum_Gothic, Gowun_Batang } from 'next/font/google'
    ...
    <div>
      <div className={editStyles.editWrapper}>
          ...
          <div className={textStyles.fontsWrapper}>
            ...
            >
              {fonts.map((fontObject, key)=>
                <div
                  key={key}
                  onClick={()=>{handleFontClick(fontObject)}} // onscroll, target scrollTop / offsetheight = i
                  className={`${textStyles.fontWrapper} ${fontObject.class.className}`}
                >
                  <p
                    className={font.name==fontObject.name?textStyles.selected:""}
                    style={{
                      margin:0,
                      width:"100%",
                  }}>
        ...
        <div
          ...
          <input
            type="text"
            className={`${textStyles.textInput} ${font.class.className} ${bold?textStyles.bold:""} ${strike?textStyles.strike:""} ${underline?textStyles.underline:""}`}
