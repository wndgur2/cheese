...
def dehaze_image(image_path):
    ...
    cv2.imshow('input image', HazeImg)                 # 원본 탁한 이미지 출력
    cv2.imshow('enhanced_image', HazeCorrectedImg)     # 결과 이미지 출력
...
cv2.imshow('img', dehaze_image('./download.jpeg'))

cv2.waitKey(0)
cv2.destroyAllWindows()

if __name__ == "__main__":
    dehaze_image('./download.jpeg')