		...
		try {
			...
			switch (message.getType()) {
				...
				case MSG_TYPE_CAMERA_ICE:
					Room camera_rm = sessionIdToRoomMap.get(session.getId());
					...
					if (camera_rm != null) {
						...
						Map<String, WebSocketSession> deviceCameraClient = roomService.getDeviceClient(camera_rm);
				...
				case MSG_TYPE_DEVICE_CAMERA_ICE:
					Room device_camera_rm = sessionIdToRoomMap.get(session.getId());
					
					if (device_camera_rm != null) {
						...
						Map<String, WebSocketSession> cameraClient = roomService.getCameraClient(device_camera_rm);
				...
				case MSG_TYPE_PRINTER_OFFER:
				case MSG_TYPE_PRINTER_ANSWER:
				case MSG_TYPE_PRINTER_ICE:
					Room printer_rm = sessionIdToRoomMap.get(session.getId());
					
					if (printer_rm != null) {
						Object printer_candidate = message.getCandidate();
						Object printer_sdp = message.getSdp();
						System.out.println(String.format("[ws_printer] Signal: %s",
								printer_candidate != null
										? printer_candidate.toString().substring(0, 64)
									: printer_sdp.toString().substring(0, 64)));

						Map<String, WebSocketSession> devicePrinterClient = roomService.getDeviceClient(printer_rm);
						
						if (!devicePrinterClient.isEmpty()) {
							Map.Entry<String, WebSocketSession> client = devicePrinterClient.entrySet().iterator().next();
							sendMessage(client.getValue(),
									new WebSocketMessage(
											userName,
											message.getType(),
				...
				case MSG_TYPE_DEVICE_PRINTER_OFFER:
				case MSG_TYPE_DEVICE_PRINTER_ANSWER:
				case MSG_TYPE_DEVICE_PRINTER_ICE:
					Room device_printer_rm = sessionIdToRoomMap.get(session.getId());

					if (device_printer_rm != null) {
						Object device_printer_candidate = message.getCandidate();
						Object device_printer_sdp = message.getSdp();
						System.out.println(String.format("[ws_device_printer] Signal: %s",
								device_printer_candidate != null
										? device_printer_candidate.toString().substring(0, 64)
										: device_printer_sdp.toString().substring(0, 64)));
						
						Map<String, WebSocketSession> printerClient = roomService.getPrinterClient(device_printer_rm);
						
						if (!printerClient.isEmpty()) {
							Map.Entry<String, WebSocketSession> client = printerClient.entrySet().iterator().next();
							sendMessage(client.getValue(),
									new WebSocketMessage(
											userName,
											message.getType(),
											data,
											device_printer_candidate,
											device_printer_sdp));
						}
					}
					break;
